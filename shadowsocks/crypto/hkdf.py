...
def hkdf_expand(pseudo_random_key, info=b'', length=32,
    ...
    for counter in range(blocks_needed):
        output_block = hmac.new(
            pseudo_random_key,
            buffer(output_block + info + bytes((counter + 1,))),
            algorithm
        ).digest()
        okm += output_block
    return okm[:length]
...
if __name__ == '__main__':
    hkdf = Hkdf(b'012345678901', b'abc123')
    key = hkdf.expand(b"ss-subkey")
    assert key == b'F\xdd\\q\xf6\x7f\x94B\xbf\xb2G\xbch\x8f\x1a("\x05\x8e`\x1bg\xf0\xd5z\xc5x\xc7\x1eT\x00\x8a'